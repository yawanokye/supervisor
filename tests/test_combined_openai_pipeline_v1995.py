...
def _config(monkeypatch):
    ...
    monkeypatch.setenv('OPENAI_CLEANING_MODEL', 'gpt-4.1-nano')
    monkeypatch.setenv('OPENAI_SECTION_ANALYSIS_MODEL', 'gpt-5.6-luna')
    monkeypatch.setenv('OPENAI_SECTION_ANALYSIS_FALLBACK_MODEL', 'gpt-5.4-mini')
    monkeypatch.setenv('OPENAI_FINAL_SYNTHESIS_MODEL', 'gpt-5.5')
...
def test_phase_1_routes_to_cleaning_model(monkeypatch):
    ...
    assert plan.primary.model == 'gpt-4.1-nano'
...
def test_phase_2_routes_to_section_analysis_model(monkeypatch):
    ...
    assert plan.fallback and plan.fallback.model == 'gpt-5.4-mini'
...
def test_phase_3_routes_to_final_synthesis_model(monkeypatch):
    ...
    assert plan.primary.provider is ProviderName.OPENAI
    assert plan.primary.model == 'gpt-5.5'
